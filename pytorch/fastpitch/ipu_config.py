...
def build_ipu_config(seed=1234, gradient_accmulate_num=128, availble_memory_proportion=0.1):
    ipu_config = poptorch.Options()
    ipu_config.autoRoundNumIPUs(True)
    ipu_config.randomSeed(seed)
    ipu_config.Training.gradientAccumulation(gradient_accmulate_num)
    ...
    ipu_config.setAvailableMemoryProportion({f'IPU{i}': availble_memory_proportion for i in range(2)})
